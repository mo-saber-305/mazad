...
class MerchantsTransactionsReport implements FromCollection, WithHeadings
{
    use Exportable;
    ...
    {
        $transactions = Transaction::where('merchant_id', '!=', 0)->with('merchant')->orderBy('id', 'desc')->get();
        $general = GeneralSetting::first();
        return $transactions->map(function ($trx) use ($general) {
            return [
                'User' => $trx->merchant->fullname,
                'Username' => $trx->merchant->username,
                'Trx' => $trx->trx,
