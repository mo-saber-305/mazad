...
@section('content')
    <div class="account-section bg_img"
         data-background="{{ getImage('assets/images/frontend/register/'.$register->data_values->background_image, '1920x1080') }}">
                                ...
                                <div class="d-flex flex-wrap">
                                    @foreach($interests as $key => $interest)
                                        <div class="form-check col-sm-6">
                                            <input class="form-check-input" type="checkbox" id="interests{{$key}}" name="interests[]"
                                                   value="{{ $interest->id }}">
                                            <label class="form-check-label" for="interests{{$key}}">{{ $interest->name }}</label>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                                ...
                                <label for="agree" class="ms-1">@lang('I agree with')
                                    @foreach ($policyPages as $policyPage)
                                        <a href="{{ route('policy', [$policyPage, slug($policyPage->data_values->title)]) }}" target="_blank"
                                           class="text--base">
                                            {{ __($policyPage->data_values->title) }}@if(!$loop->last)
                                                ,
                                            @endif
                                        </a>
                                    @endforeach
