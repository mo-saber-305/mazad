        ...
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-body ">
                    ...
                    <form action="{{ route('admin.merchant.ticket.reply', $ticket->id) }}" enctype="multipart/form-data" method="post" class="form-horizontal">
                        @csrf
                                    ...
                                    @if($message->attachments->count() > 0)
                                        <div class="my-3">
                                            @foreach($message->attachments as $k=> $image)
                                                <a href="{{route('admin.merchant.ticket.download',encrypt($image->id))}}" class="mr-3"><i class="fa fa-file"></i>@lang('Attachment') {{++$k}}</a>
                                            @endforeach
                                        </div>
                                    @endif
                                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form method="post" action="{{ route('admin.merchant.ticket.reply', $ticket->id) }}">
                        @csrf

                        <button type="submit" class="btn btn--primary btn-block" name="replayTicket" value="2"> @lang('Close Ticket') </button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="DelMessage" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form method="post" action="{{ route('admin.merchant.ticket.delete')}}">
                        @csrf
                        <input type="hidden" name="message_id" class="message_id">
                        <button type="button" class="btn btn--dark" data-dismiss="modal">@lang('No') </button>
                        <button type="submit" class="btn btn--primary"><i class="fa fa-trash"></i> @lang('Yes') </button>
                    </form>
                </div>
...
@push('breadcrumb-plugins')
    <a href="{{ route('admin.merchant.ticket') }}" class="btn btn-sm btn--primary box--shadow1 text--small"><i class="la la-fw la-backward"></i> @lang('Go Back') </a>
@endpush
