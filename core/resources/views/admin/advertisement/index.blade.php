                            ...
                            @forelse($advertisements as $k=> $advertisement)
                                <tr id={{ 'row_' . $advertisement->id }}>
                                    ...
                                    <td data-label="@lang('value')">
                                        ...
                                        @if (@$advertisement->type == 'image')
                                            <img id="image__{{ $advertisement->id }}"
                                                 src="{{ getImage(imagePath()['advertisement']['path'] . '/' . @$advertisement->value) }}"
                                                 alt="" class="max-w-50">
                                        @elseif (@$advertisement->type == 'video')
                                            <span class="badge badge--primary" id="video__{{ $advertisement->id }}"
                                                  data-src="{{ getImage(imagePath()['advertisement']['path'] . '/' . @$advertisement->value) }}">
                                                @lang('video')
                                            </span>
                                        @else
                                            <span class="badge badge--warning">@lang('Script')</span>
                                        @endif
                                    ...
                                    <td>
                                        ...
                                        <button class="icon-btn btn--danger deleteBtn" data-id="{{ $advertisement->id }}"><i class="la la-trash"></i></button>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                ...
                <form class="form-horizontal" method="post" action="{{ route('admin.advertisement.store') }}"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-12" id="__video">
                                <div class="form-group">

                                    <div class="video-upload mt-3">
                                        <div class="thumb">
                                            <div class="avatar-preview">
                                                <label for="" class="font-weight-bold">@lang('Video') <strong
                                                            class="text-danger">*</strong></label>

                                                <video width="500" controls style="display: block">
                                            ...
                                            <div class="avatar-edit">
                                                <input type="file" size-validation="" class="profilePicUpload d-none"
                                                       name="video" id="videoUpload" accept=".mp4, .mov, .ogg, .qt, .flv, .3gp, .avi, .wmv">
                                                <label for="videoUpload" class="bg--primary mt-3">@lang('Upload
                                            Video')</label>
                                                <small class="mt-2 text-facebook">@lang('Supported files'):
                                                    <b>@lang('mp4,mov,ogg,qt,flv,3gp,avi,wmv') <span id="__video_size"></span></b>
                            ...
                            <div class="col-lg-12">
                                <div class="form-group statusGroup">
                                    ...
                                    <input type="checkbox" data-onstyle="-success" data-offstyle="-danger" data-toggle="toggle" data-on="@lang('Active')"
                                           data-off="@lang('Inactive')" data-width="100%" name="status">
                                </div>
                            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{route('admin.advertisement.delete')}}" method="POST">
                    ...
                    <div class="modal-body">
                        <p>@lang('Are you sure to') <span class="font-weight-bold">@lang('delete')</span> @lang('this advertisement') <span
...
@push('breadcrumb-plugins')
    <a class="btn btn-sm btn--primary box--shadow1 text-white text--small __advertisement" href="#"><i
                class="fa fa-fw fa-plus"></i>@lang('New Advertisement')</a>
@endpush
...
@push('style')
    <style>
        ...
        .video-upload .thumb .profilePicPreview {
            max-width: 100%;
        }
        ...
        .video-upload .thumb .avatar-edit label {
            text-align: center;
            line-height: 45px;
            font-size: 18px;
            cursor: pointer;
        ...
        .video-upload .thumb .profilePicPreview .remove-video {
            position: absolute;
            top: -9px;
            right: -9px;
            text-align: center;
        ...
        (function ($) {
            ...
            $(document).on('change', '#__type', function (e) {
                ...
                if (value == 'script') {
                    ...
                    $(document).find('#__video').css('display', 'none');
                    ...
                    $(document).find('#__image').css('display', 'none');
                    $(document).find('#__script').css('display', 'none');
                    $(document).find('#__video').css('display', 'block');
                }
            ...
            $(document).on('change', '#__size', function (e) {
                ...
                if (type == "image") {
                    ...
                    $(document).find('#__image_size').text(`, Upload Image Size Must Be ${size} px`);
                    $(document).find("#imageUpload").attr('size-validation', size.val())
                    changeImagePreview()
                }

                if (type == "video") {
                    $(document).find('.video-upload').css('display', 'block')
                    $(document).find('#__image').css('display', 'none')
                }
            ...
            $(document).on('click', '.editBtn', function (e) {
                ...
                $(modal).find('#__size').val(advertisement.size || "")
                modal.find("#__type").val(advertisement.type)
                ...
                if (advertisement.type == "image") {
                    ...
                    $(document).find('#__image').css('display', 'none');
                    $(document).find('#__script').css('display', 'none');
                    let video_src = $(document).find("#video__" + advertisement.id).attr('data-src');
                    // $(modal).find('#__video .video-upload video #video_here').attr('src', video_src);
