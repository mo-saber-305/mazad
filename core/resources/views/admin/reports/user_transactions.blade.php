                            ...
                            @forelse($transactions as $trx)
                                <tr>
                                    <td data-label="@lang('User')">
                                        ...
                                        <span class="small"> <a href="{{ route('admin.users.detail', $trx->user_id) }}"><span>@</span>{{ $trx->user->username }}
                                            </a> </span>
...
@push('breadcrumb-plugins')
    <div class="d-flex flex-wrap justify-content-sm-end ">
        ...
        @else
            <form action="{{ route('admin.report.user.transaction.search') }}" method="GET" class="form-inline float-sm-right bg--white">
                <div class="input-group has_append">
                    <input type="text" name="search" class="form-control" placeholder="@lang('TRX / Username')" value="{{ $search ?? '' }}">
                    <div class="input-group-append">
                        <button class="btn btn--primary" type="submit"><i class="fa fa-search"></i></button>
        ...
        <div class="dropdown ml-3">
            ...
            <div class="dropdown-menu text-center">
                <a class="dropdown-item"
                   href="{{ route('admin.export.users-transactions-report', ['file_type' => 'excel']) }}">@lang('Excel')</a>
                <a class="dropdown-item"
                   href="{{ route('admin.export.users-transactions-report', ['file_type' => 'csv']) }}">@lang('Csv')</a>
            </div>
        </div>
    </div>
